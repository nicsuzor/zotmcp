...
class TestGetItemFullText:
    ...
    async def test_get_item_always_returns_temp_file(self, mcp_server):
        """get_item should ALWAYS return full_text_file, not inline text.

        Consistent behavior: every call returns a temp file path.
        Client can then choose to read inline via get_full_text if needed.

        This test uses a SMALL document (few chunks) to verify that even
        small documents get temp file handling, not inline text.

        Expected failure: Current code returns `full_text` inline for small
        docs (<500KB) - test should fail because `full_text_file` won't be
        present for small documents.
        """
        from pathlib import Path

        async with Client(mcp_server) as client:
            # Use same query as working test
            search_result = await client.call_tool(
                "search",
                {
                    "query": "content moderation",
            ...
            # Get the first result - any document works for this test
            first_result = results[0]
            zotero_key = first_result.get("zotero_key")
            assert zotero_key, f"Expected 'zotero_key' in search result: {first_result}"
            ...
            # EXPECTED: full_text_file should ALWAYS be present
            # This assertion will FAIL with current code that returns inline full_text
            assert "full_text_file" in item_data, (
                f"get_item should ALWAYS return 'full_text_file' for document "
                f"'{zotero_key}' (chunk_count={item_data.get('chunk_count')}). "
                f"Got keys: {list(item_data.keys())}. "
                f"Current behavior returns inline 'full_text' for small docs."
            )

            # EXPECTED: full_text should NOT be in response (consistent behavior)
            assert "full_text" not in item_data, (
                f"get_item should NOT return inline 'full_text'. "
                f"All documents should use temp files for consistency. "
                f"Got keys: {list(item_data.keys())}"
            )
            ...
            assert len(file_content) > 0, (
                f"Temp file should have content, got empty file"
            )
...
class TestGetItemLargeDocument:
    """Tests for document handling with temp file output.

    NOTE: ALL documents now return temp files (not just large ones).
    This class tests behavior with genuinely large documents (>500KB).
    """

    # Threshold for "large" document (500KB) - used for validation
    LARGE_DOCUMENT_THRESHOLD_BYTES = 500 * 1024

    async def test_get_item_large_document_uses_temp_file(self, mcp_server):
        """Large documents (>500KB) should return temp file path.

        Uses mcp_server (parametrized): Validates get_item through MCP client
        interface in both local and Docker environments.

        All documents now return temp files, but this test specifically
        validates behavior with genuinely large documents:
        ...
        async with Client(mcp_server) as client:
            ...
            for result in results:
                ...
                # Check document size via temp file
                if "full_text_file" in item_data:
                    temp_path = Path(item_data["full_text_file"])
                    if temp_path.exists():
                        file_size = temp_path.stat().st_size
                        if file_size > largest_size:
                            largest_size = file_size
                        if file_size > self.LARGE_DOCUMENT_THRESHOLD_BYTES:
            ...
            if not large_doc_found:
                pytest.skip(
                    ...
                    f"Largest found was {largest_size} bytes. "
